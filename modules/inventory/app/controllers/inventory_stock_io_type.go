...
func (c InventoryStockIoType) ListData() revel.Result {
	...
	stock_io_type_list := make([]models.StockIoType, 0, limit)
	err := data_query.Limit(limit, offset).Find(&stock_io_type_list)
	core.HandleError(err)

	count_query := *query
	count, err := count_query.Count(new(models.StockIoType))
	core.HandleError(err)
...
type StockIoTypeDetailForm struct {
	StockIoType models.StockIoType `json:"iotype"`
}
...
func (c InventoryStockIoType) DetailData() revel.Result {
	...
	var stock_io_type models.StockIoType
...
func (c InventoryStockIoType) Delete() revel.Result {
	...
	affected, err := db_session.In("id", stock_io_type_id_list).Delete(new(models.StockIoType))
	core.HandleError(err)
