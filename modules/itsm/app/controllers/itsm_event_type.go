...
func (c ItsmEventType) ListData() revel.Result {
	...
	event_type_list := make([]models.EventType, 0, limit)
	err := data_query.Limit(limit, offset).Find(&event_type_list)
	core.HandleError(err)

	count_query := *query
	count, err := count_query.Count(new(models.EventType))
	core.HandleError(err)
...
func (c ItsmEventType) DetailData() revel.Result {
	...
	var event_type models.EventType
	if event_type_id != 0 {
...
func (c ItsmEventType) Delete() revel.Result {
	...
	affected, err := db_session.In("id", event_type_id_list).Delete(new(models.EventType))
	core.HandleError(err)
