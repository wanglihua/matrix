...
func (c ItsmApplication) ListData() revel.Result {
	...
	application_list := make([]models.Application, 0, limit)
	err := data_query.Limit(limit, offset).Find(&application_list)
	core.HandleError(err)

	count_query := *query
	count, err := count_query.Count(new(models.Application))
	core.HandleError(err)
...
func (c ItsmApplication) Delete() revel.Result {
	...
	affected, err := db_session.In("id", application_id_list).Delete(new(models.Application))
	core.HandleError(err)
