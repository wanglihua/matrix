...
func (c ItsmServiceArea) ListData() revel.Result {
	...
	service_area_list := make([]models.ServiceArea, 0, limit)
	err := data_query.Limit(limit, offset).Find(&service_area_list)
	core.HandleError(err)

	count_query := *query
	count, err := count_query.Count(new(models.ServiceArea))
	core.HandleError(err)
...
type ServiceAreaDetailForm struct {
	ServiceArea models.ServiceArea `json:"area"`
}
...
func (c ItsmServiceArea) DetailData() revel.Result {
	...
	var service_area models.ServiceArea
...
func (c ItsmServiceArea) Delete() revel.Result {
	...
	affected, err := db_session.In("id", service_area_id_list).Delete(new(models.ServiceArea))
	core.HandleError(err)
