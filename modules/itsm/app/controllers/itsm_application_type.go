...
func (c ItsmApplicationType) ListData() revel.Result {
	...
	application_type_list := make([]models.ApplicationTypeInfo, 0, limit)
	err := data_query.Limit(limit, offset).Find(&application_type_list)
	core.HandleError(err)

	count_query := *query
	count, err := count_query.Count(new(models.ApplicationTypeInfo))
	core.HandleError(err)
...
type ApplicationTypeDetailForm struct {
	ApplicationType models.ApplicationTypeInfo `json:"application_type"`
}
...
func (c ItsmApplicationType) DetailData() revel.Result {
	...
	var application_type models.ApplicationTypeInfo
...
func (c ItsmApplicationType) Delete() revel.Result {
	...
	affected, err := db_session.In("id", application_type_id_list).Delete(new(models.ApplicationTypeInfo))
	core.HandleError(err)
